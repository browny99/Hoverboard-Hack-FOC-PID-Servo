...
// Matlab defines - from auto-code genreation
//---------------
...
extern void
Motor_Pos(); // vam calculate cumulation of encoder positions left and right
...
int main(void) {
  ...
  while (HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN))
    HAL_Delay(10); // keeps from entering auto_cal with long button push
  ...
  while (1) {
    if (update_periodic_loop(DELAY_IN_MAIN_LOOP)) {
      ...
      // ####### MOTOR ENABLING: Only if the initial input is
      //         very small (for SAFETY) #######
      if (enable == 0) {
      ...
      // Motor_Pos( &MotorPosL, &MotorPosR); // Calculate motor electrical
      // positions relative to startup positions
      //  ####### LOW-PASS FILTER #######
      ...
      // speed1 = ( main_loop_counter / 10)*500;
      // speed2 = ( main_loop_counter / 10)*500;
      ...
      #endif
      // scale to 2000 units per rotation   sf = .37  =2000/(360 deg*15pole pairs= 5400 elec deg)
      PIDL.feedback = (MotorPosL * 2000) / 5400;
      // minimum step is 60 deg elec phase angle, or 4 deg mechanical angle 
      PIDR.feedback = (MotorPosR * 2000) / 5400;
      ...
      #if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
      {
        ...
        if (main_loop_counter % 1 == 0) { //  Send PID data periodically every 5 ms
          printf("IL %i FL %i IR %i FR %i\r\n", PIDL.input, PIDL.feedback, PIDR.input, PIDR.feedback);
        }
      }
      #endif
        ...
        if (main_loop_counter % 2 == 0) { // Send data periodically every 10 ms
          ...
          Feedback.batVoltage =
              (int16_t)(batVoltage * BAT_CALIB_REAL_VOLTAGE / BAT_CALIB_ADC);
      ...
      // HAL_GPIO_TogglePin(LED_PORT, LED_PIN);                 // This is to
      // measure the main() loop duration with an oscilloscope connected to
      // LED_PIN Update main loop states
      cmdL_prev = cmdL;
      cmdR_prev = cmdR;
      main_loop_counter++;
    }
